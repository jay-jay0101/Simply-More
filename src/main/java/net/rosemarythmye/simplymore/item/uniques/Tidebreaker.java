...
public class Tidebreaker extends UniqueSword {
    ...
    public Tidebreaker(ToolMaterial toolMaterial, int attackDamage, float attackSpeed, Settings settings) {
    ...
    public boolean postHit(ItemStack stack, LivingEntity target, LivingEntity attacker) {
        if (!attacker.getWorld().isClient()) {
            if (attacker.getRandom().nextInt(100) <= 25) {
                if(!attacker.hasStatusEffect(ModEffects.TIDEBREAKER)) {
                    attacker.addStatusEffect(new StatusEffectInstance(ModEffects.TIDEBREAKER, 300, 0), attacker);
                }
            }
