...
public class Holylight extends UniqueSword {

    int skillCooldown = 800;

    public Holylight(ToolMaterial toolMaterial, int attackDamage, float attackSpeed, Settings settings) {
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        if (!user.getWorld().isClient()) {
            for (LivingEntity livingEntity : user.getWorld().getNonSpectatingEntities(LivingEntity.class,new Box(user.getX()-10,user.getY()-10,user.getZ()-10,user.getX()+10,user.getY()+10,user.getZ()+10))) {
                ...
                livingEntity.addStatusEffect(new StatusEffectInstance(ModEffects.BLESSING, 160));
            }

            user.getWorld().playSound(null, user.getBlockPos(), SoundEvents.ENTITY_EVOKER_PREPARE_SUMMON, user.getSoundCategory(), 2F, 1.5F);
            ((ServerWorld) user.getWorld()).spawnParticles(ParticleTypes.WAX_OFF,user.getX(),user.getY()+1,user.getZ(),50,0.25,0.5,0.25,0.1);
