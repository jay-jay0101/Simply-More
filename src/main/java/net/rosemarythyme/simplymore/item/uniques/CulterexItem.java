...
public class CulterexItem extends SimplyMoreUniqueSwordItem {
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        if(entity instanceof LivingEntity target) {
            ...
            if(target.hasStatusEffect(ModEffectsRegistry.HEX)) {
                int amplifier = target.getStatusEffect(ModEffectsRegistry.HEX).getAmplifier() + 1;
                target.removeStatusEffect(ModEffectsRegistry.HEX);

                user.addStatusEffect(
                        new StatusEffectInstance(
                                ModEffectsRegistry.SOUL_HEALTH,
                                amplifier * 100,
                                amplifier - 1
                        )
                );
    ...
    public boolean postHit(ItemStack stack, LivingEntity target, LivingEntity attacker) {
        ...
        if(target.hasStatusEffect(ModEffectsRegistry.HEX)
            ...
            int duration = target.getStatusEffect(ModEffectsRegistry.HEX).getDuration();
            int amplifier = target.getStatusEffect(ModEffectsRegistry.HEX).getAmplifier();
