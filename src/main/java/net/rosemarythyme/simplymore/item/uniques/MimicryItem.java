...
public class MimicryItem extends SimplyMoreUniqueSwordItem {
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        if(user.hasStatusEffect(ModEffectsRegistry.MIMICRY_HAPPENING)) {
    ...
    public void usageTick(World world, LivingEntity user, ItemStack stack, int remainingUseTicks) {
        ...
        if(remainingUseTicks == 1 && ModItemsRegistry.MIMICRY_AMPLIFIERS.contains(stack.getItem())) {
            user.stopUsingItem();
            ...
            user.addStatusEffect(
                    new StatusEffectInstance(
                            ModEffectsRegistry.MIMICRY_HAPPENING,
                            usageEffectTime,
                            ModItemsRegistry.MIMICRY_AMPLIFIERS.indexOf(stack.getItem())
                    )
            );
    ...
    public void inventoryTick(ItemStack stack, World world, Entity entity, int slot, boolean selected) {
        ...
        if (stack.getOrCreateNbt().contains("simplymore:change")
                ...
                && !playerEntity.hasStatusEffect(ModEffectsRegistry.MIMICRY_HAPPENING)
                && !playerEntity.getWorld().isClient
        ) {
            playerEntity.getItemCooldownManager().set(stack.getItem(), skillCooldown);
            String currentForm = null;

            for (Map.Entry<String, Item> itemEntry : ModItemsRegistry.MIMICRY_ITEMS.entrySet()) {
                if(itemEntry.getValue() == stack.getItem()) {
                    currentForm = itemEntry.getKey();
                }
            }

            String newForm = getWeightedRandomForm(currentForm, playerEntity);
            Item newItem = ModItemsRegistry.MIMICRY_ITEMS.get(newForm);
    ...
    public boolean onClicked(ItemStack stack, ItemStack otherStack, Slot slot, ClickType clickType, PlayerEntity player, StackReference cursorStackReference) {
        if(otherStack.isIn(ModTagRegistry.ALL) && !player.hasStatusEffect(ModEffectsRegistry.MIMICRY_HAPPENING)) {
            String clickedItemType = checkItem(otherStack.getItem());
            Item itemToTransformInto = ModItemsRegistry.MIMICRY_ITEMS.get(clickedItemType);
    ...
    public String getWeightedRandomForm(String currentForm, PlayerEntity player) {
        ...
        for (int i = 0; i < 36; i++) {
            ...
            if(itemStack.isIn(ModTagRegistry.ALL)) {
                ...
                if(formString != null && isFormEnabled(((MimicryItem) ModItemsRegistry.MIMICRY_ITEMS.get(formString)), player)) {
        ...
        if(availableForms.isEmpty()) {
            for(Map.Entry<String, Item> itemEntry : ModItemsRegistry.MIMICRY_ITEMS.entrySet()) {
                ...
                Item formItem = itemEntry.getValue();
