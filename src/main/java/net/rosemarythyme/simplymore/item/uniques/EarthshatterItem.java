    ...
    public boolean postHit(ItemStack stack, LivingEntity target, LivingEntity attacker) {
            if (!attacker.getWorld().isClient()) {
                if (attacker.getRandom().nextInt(100) <= 15) {
                    if(target.hasStatusEffect(ModEffectsRegistry.ARMOUR_CRUNCH)) {
                        int amplifier = target.getStatusEffect(ModEffectsRegistry.ARMOUR_CRUNCH).getAmplifier()+1;
                        target.addStatusEffect(new StatusEffectInstance(ModEffectsRegistry.ARMOUR_CRUNCH, 200, amplifier), attacker);
                    } else {
                        target.addStatusEffect(new StatusEffectInstance(ModEffectsRegistry.ARMOUR_CRUNCH, 200, 0), attacker);
                    }
                }
            }
