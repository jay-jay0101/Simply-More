...
public class BladeOfTheGrotesqueItem extends SimplyMoreUniqueSwordItem {
    ...
    public static void causeStun(PlayerEntity player) {
        ...
        for (LivingEntity livingEntity : livingEntities) {
            ...
            livingEntity.addStatusEffect(new StatusEffectInstance(ModEffectsRegistry.STUNNED,effect.getGrotesqueSolidifyAuraStunTime()),player);
        }
    }
    ...
    public void inventoryTick(ItemStack stack, World world, Entity entity, int slot, boolean selected) {
        if (world.getTime() % 40 == 0 && entity instanceof ServerPlayerEntity player && selected) {
            ...
            for (LivingEntity livingEntity : livingEntities) {
                ...
                if(livingEntity.hasStatusEffect(ModEffectsRegistry.GROTESQUE_WARD)) {
                    int amplifier = livingEntity.getStatusEffect(ModEffectsRegistry.GROTESQUE_WARD).getAmplifier();
                    amplifier = (int) Math.min(amplifier + 1, effect.getGrotesqueMaxAuraWard());
                    livingEntity.addStatusEffect(new StatusEffectInstance(ModEffectsRegistry.GROTESQUE_WARD,50,amplifier));
                } else {
                    livingEntity.addStatusEffect(new StatusEffectInstance(ModEffectsRegistry.GROTESQUE_WARD,50,0));
                }
            }
        }
