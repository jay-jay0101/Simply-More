...
public class CrowEntity extends TameableEntity implements Ownable {
    ...
    public void tick() {
        ...
        if (player != null) {
            if (player.getItemCooldownManager().isCoolingDown(ModItemsRegistry.DEATHS_EYRIE)) {
                if(this.getAttackingTime() >= -1) {
                    ...
                    if (time % 10 == 0) {
                        ...
                        target.addStatusEffect(new StatusEffectInstance(ModEffectsRegistry.BLEED,effect.getDeathsEyrieCrowAttackBleedTime()));
