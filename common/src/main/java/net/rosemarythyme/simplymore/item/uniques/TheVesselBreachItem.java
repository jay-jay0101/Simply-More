...
public class TheVesselBreachItem extends SimplyMoreUniqueSwordItem {
    ...
    public boolean postHit(ItemStack stack, LivingEntity target, LivingEntity attacker) {
            if (!attacker.getWorld().isClient()) {
                if (!(target instanceof ArmorStandEntity)) {
                    if (!attacker.hasStatusEffect(ModEffectsRegistry.RAGE.get())) {
                        ...
                        target.addStatusEffect(new StatusEffectInstance(ModEffectsRegistry.BLEED.get(), effect.getVesselRageBleedTime(),0));
                    }
                }
            }
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        if (!user.getWorld().isClient) {
            ...
            user.addStatusEffect(new StatusEffectInstance(ModEffectsRegistry.RAGE.get(), effect.getVesselRageTime(), 0));
