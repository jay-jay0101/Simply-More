...
public class TheBloodHarvesterItem extends SimplyMoreUniqueSwordItem {
            ...
            if (!attacker.getWorld().isClient()) {
                if (!(target instanceof ArmorStandEntity)) {
                    if (!attacker.hasStatusEffect(ModEffectsRegistry.getReference(ModEffectsRegistry.HARVEST))) {
                        ...
                        target.addStatusEffect(new StatusEffectInstance(ModEffectsRegistry.getReference(ModEffectsRegistry.BLEED), UNIQUE_CONFIG.the_blood_harvester.bleedTime,0));
                    }
                }
            }
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        if (!user.getWorld().isClient) {
            user.addStatusEffect(new StatusEffectInstance(ModEffectsRegistry.getReference(ModEffectsRegistry.HARVEST), UNIQUE_CONFIG.the_blood_harvester.harvestTime, 0));
    ...
    public void appendTooltip(ItemStack itemStack, TooltipContext tooltipContext, List<Text> tooltip, TooltipType type) {
        ...
        tooltip.add(Text.translatable("item.simplymore.the_blood_harvester.tooltip2",
                MathUtils.toPercentage(UNIQUE_CONFIG.the_blood_harvester.lifesteal)).setStyle(textStyle));
        tooltip.add(Text.literal(""));
        ...
        tooltip.add(Text.translatable("item.simplymore.the_blood_harvester.tooltip3",
                ...
                MathUtils.toPercentage(UNIQUE_CONFIG.the_blood_harvester.harvestLifesteal)
        ).setStyle(textStyle));
