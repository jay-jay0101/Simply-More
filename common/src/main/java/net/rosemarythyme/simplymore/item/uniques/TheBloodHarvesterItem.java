...
public class TheBloodHarvesterItem extends SimplyMoreUniqueSwordItem {
    ...
    public boolean postHit(ItemStack stack, LivingEntity target, LivingEntity attacker) {
            if (!attacker.getWorld().isClient()) {
                if (!(target instanceof ArmorStandEntity)) {
                    if (!attacker.hasStatusEffect(ModEffectsRegistry.HARVEST.get())) {
                        ...
                        target.addStatusEffect(new StatusEffectInstance(ModEffectsRegistry.BLEED.get(),effect.getHarvesterBleedTime(),0));
                    }
                }
            }
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        if (!user.getWorld().isClient) {
            user.addStatusEffect(new StatusEffectInstance(ModEffectsRegistry.HARVEST.get(), effect.getHarvesterHarvestTime(), 0));
    ...
    public void appendTooltip(ItemStack itemStack, World world, List<Text> tooltip, TooltipContext tooltipContext) {
        ...
        tooltip.add(Text.translatable("item.simplymore.the_blood_harvester.tooltip3",
                SimplyMoreHelperMethods.translateTicks(effect.getHarvesterHarvestTime())).setStyle(textStyle));
